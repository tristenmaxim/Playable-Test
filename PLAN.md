...
### 1.3 Фон
- [x] Скопировать `asset_0015_background_1.png` в `assets/images/`
- [x] Добавить загрузку текстуры в Background.js
- [x] Создать спрайт фона и добавить на сцену
- [x] Реализовать скролл фона с бесшовным зацикливанием (оригинал-зеркально)
- [x] Добавить правильное масштабирование с сохранением пропорций
- [x] Добавить деревья, кусты и фонари на одной линии
- [x] Правильный порядок по глубине: деревья (дальше) -> фонари -> кусты (ближе)
- [x] Корректное зеркальное отображение для второй секции
- [x] Настроены размеры элементов
- [x] Проверено отображение в браузере
- [x] Коммит: "feat: background with trees, bushes and lamps completed"
...
- [x] Изучить спрайт `asset_0004_hero.png`
- [ ] Конвертировать в base64 data URI (будет сделано на этапе сборки)
- [ ] Встроить в HTML (будет сделано на этапе сборки)

### 2.2 Анимация бега
- [x] Создать спрайт героя
- [x] Реализовать анимацию бега (AnimatedSprite из спрайт-листа 932x1506px)
- [x] Разместить героя на правильной позиции (нижняя часть экрана, GROUND_Y: 450)
- [x] Настроить размер спрайта (масштабирование до ~80px высоты)

### 2.3 Управление - прыжок
- [x] Добавить обработчик клика/тапа
- [x] Реализовать физику прыжка (гравитация, скорость)
- [ ] Добавить анимацию прыжка (можно добавить позже)
- [x] Ограничить прыжок землей (не давать улететь вверх)
- [x] Проверить работу прыжка
- [x] Коммит: "feat: add hero with run animation and jump mechanics"
...
**Текущий этап**: Этап 2 - Герой - бег и прыжки ✅ ЗАВЕРШЕН

**Последнее обновление**: 2026-01-17

**Следующий шаг**: Этап 3 - Добавить деньги (сбор предметов)
